# MAXIM Agent Payment Public Payment Pages Deploy Packet

Local-only export. No public deploy, post, send, payment movement, wallet action, account/KYC action, or revenue count has occurred.
...
- Manifest digest: `1b548bb5fc42dc7b`
- Exact deploy route: `/api/agent-payment-reliability-kit/public-payment-page-deploy-workbench/run`
- Exact deploy approval phrase: `MAXIM approve Agent Payment public payment page deploy 1b548bb5fc42dc7b`
- Local export refreshed at: `2026-07-03T10:33:24.553394+00:00`

## Pages
- `x402-foundation/x402#2691` -> https://maximultra.dev/pay/agent-payment/checkout-85a63f42bb2e
- `x402-foundation/x402#2691` -> https://maximultra.dev/pay/agent-payment/checkout-bda873ad7dc1
- `AcePeak/naturo#889` -> https://maximultra.dev/pay/agent-payment/checkout-17333c31831d
- `NousResearch/hermes-agent#57533` -> https://maximultra.dev/pay/agent-payment/checkout-c58b1fc7e272
- `NousResearch/hermes-agent#57364` -> https://maximultra.dev/pay/agent-payment/checkout-fc7f992c52ba
- `senara-solutions/mika#1719` -> https://maximultra.dev/pay/agent-payment/checkout-fb70a166973e
- `explorium-ai/mcp-explorium#32` -> https://maximultra.dev/pay/agent-payment/checkout-c5de12d2b98a
- `edgarfloresguerra2011-a11y/marketnow#2` -> https://maximultra.dev/pay/agent-payment/checkout-48e3be728279
...
- `botho-project/botho#458` -> https://maximultra.dev/pay/agent-payment/checkout-aa2e9968ed9a
- `Stash-Mobile-App-Group-90/stash#3` -> https://maximultra.dev/pay/agent-payment/checkout-9e5fcae7c564
- `Smartdevs17/agenticpay#453` -> https://maximultra.dev/pay/agent-payment/checkout-772a107fc11b
...
1. Review all generated pages locally.
2. If using the app route, run the exact deploy packet only with the approval phrase above.
3. If deploying manually, copy `public_ready/` into the `maxim-ultra/proof` GitHub Pages repository only after exact owner approval.
4. Wait for GitHub Pages to build.
5. Live-check each public URL before referencing it in any public close message.
6. Rerun the same-day/positive-reply close gate and post only if the target page is live and attached.

Revenue remains `$0.00` until owner-attributed payment receipt evidence exists.
